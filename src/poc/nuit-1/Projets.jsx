// src/poc/nuit-1/Projets.jsx
// Section « Projets ». Le `dataMap` reste DÉRIVÉ (et non stocké) : les données
// changent aussi avec la langue. L'onglet actif est mémorisé dans
// `sessionStorage.activeProjectTab`, comme sur /tech.
//
...
import OngletsProjets from "./OngletsProjets";
...
const Projets = ({ contenu }) => {
  ...
  return (
    <section id="project" className="nuit1-section nuit1-projets">
      <div className="nuit1-container">
        <EnteteSection sub={nightUi.sections.projectsSub} title={nightUi.sections.projectsTitle} />

        <OngletsProjets list={list} selected={selected} onSelect={choisir} />

        <div ref={grilleRef} style={hauteurMini ? { minHeight: `${hauteurMini}px` } : undefined}>
          <AnimatePresence mode="wait" initial={false}>
            <motion.div
              key={selected}
              id="nuit1-grille-projets"
              role="tabpanel"
              aria-labelledby={`nuit1-onglet-${selected}`}
              className="nuit1-projets__grille"
              initial={{ opacity: 0 }}
