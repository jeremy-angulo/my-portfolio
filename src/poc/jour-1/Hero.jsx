// src/poc/jour-1/Hero.jsx
// Hero du POC « Trait d'union » : même balisage et mêmes classes que ProHero
// (rien ne change de place), mais tout se compose sous les yeux du visiteur.
//
...
import AmberStroke from "./AmberStroke";
...
// 0.35, 1) — vit en CSS, dans page.scss, avec les transitions qu'il pilote.
const SORTIE = [0.22, 1, 0.36, 1];
const CLE_REJOUE = "j1-hero-played";
...
const Mot = ({ mot, delai, duree, statique }) => {
  if (statique) return <span className="j1-word">{mot}</span>;
  return (
    <span className="j1-word">
      <RevealWord variant="mask" delay={delai} duration={duree} ease={SORTIE} className="j1-word__mask">
        {mot}
      </RevealWord>
    </span>
  );
...
const MotEm = ({ mot, delai, duree, trait, statique }) => {
  const interieur = (
    <>
      {mot}
      <span className="j1-word__amber" aria-hidden="true" style={statique ? undefined : trait.style}>
        {mot}
      </span>
    </>
  );
  ...
  return (
    <span className="j1-word j1-word--em">
      {statique ? (
        <span className="j1-word__mask">{interieur}</span>
      ) : (
        <RevealWord variant="mask" delay={delai} duration={duree} ease={SORTIE} className="j1-word__mask">
          {interieur}
        </RevealWord>
      )}
      <AmberStroke delay={trait.delai} duration={trait.duree} statique={statique} />
    </span>
  );
};
    ...
    <header className="pro-hero" ref={hero}>
      ...
      <div className="pro-container pro-hero__inner">
        <div>
          ...
          <h1 className={`pro-hero__title j1-hero__title${dessine ? " is-drawn" : ""}`}>
            {motsPre.map((mot, i) => (
              <Fragment key={`pre-${i}`}>
          ...
          <div className="pro-hero__ctas">
            <motion.a
              ...
              variants={montee(t0 + ms(TIMINGS.ctas[0]))}
            ...
            <motion.a
              ...
              variants={montee(t0 + ms(TIMINGS.ctas[1]))}
              initial={statique ? false : "hidden"}
              animate="show"
            >
              <FiLinkedin />
            ...
            <motion.div
              variants={montee(t0 + ms(TIMINGS.ctas[2]))}
              initial={statique ? false : "hidden"}
              animate="show"
