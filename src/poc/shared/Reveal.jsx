...
// `\n` découpent aussi). Un POC qui doit garder un `<em>` à l'intérieur d'un
...
export const RevealWord = ({
  ...
  const interieur = (
    <motion.span
      className={`poc-reveal__in${pose ? " is-settled" : ""}`}
      initial={depart}
  ...
  if (variant === "mask") {
    return (
      <span
        className={`poc-reveal__mask${className ? ` ${className}` : ""}`}
        style={style}
      >
        {interieur}
      </span>
...
const Reveal = ({
  ...
  if (by === "line") {
    contenu = enLignes(children).map((ligne, i) => (
      <span className="poc-reveal__line" key={`${i}-${ligne.slice(0, 12)}`}>
        <RevealWord
  ...
  return (
    <As ref={conteneur} className={`poc-reveal${className ? ` ${className}` : ""}`} style={style}>
