// src/night/NightHero.jsx
// Le hero de la facette nuit. Mêmes cotes que le hero du jour (le toggle
// jour/nuit ne déplace rien), mais aucun fond image : deux dégradés CSS, et
// la lumière devient un objet de la page.
//
...
import NightLamp, { FournisseurLampe } from "./NightLamp";
import NightSignature from "./NightSignature";
import NightPortrait from "./NightPortrait";
import { riseSoft } from "./nightMotion";

const SEPARATEUR_PARAGRAPHE = /<br\s*\/?>\s*<br\s*\/?>/i;

const NightHero = ({ ui, lang }) => {
  ...
  return (
    <section className="night-hero" ref={heroRef}>
      <span className="night-hero__ondes" aria-hidden="true" />

      <FournisseurLampe heroRef={heroRef} lang={lang}>
        <NightLamp eclose={signe} />

        <div className="night-container night-hero__inner">
          <motion.div
            className="night-hero__col"
            style={sortieActive ? { opacity: opaciteCol, y: yCol } : undefined}
          >
            <motion.span
              className="night-hero__badge"
            ...
            <h1 className="night-hero__titre">
              <span className="night-hero__titre-ligne">
                <Reveal
                  as="span"
                  by="word"
                  variant="mask"
              ...
              <span className="night-hero__titre-ligne">
                ...
                <NightSignature key={lang} texte={ui.name} onSigne={() => setSigne(true)} />
              </span>
            </h1>

            <div className="night-hero__typed">
              {reduit ? (
                <span className="night-hero__typed-fixe">{ui.typed[0]}</span>
              ) : (
                <motion.span
            ...
            <div className="night-hero__intro">
              {introDecoupee ? (
            ...
            <div className="night-hero__ctas">
              {[
                <button
                  ...
                  type="button"
                  onClick={versProjets}
                  className="night-btn night-btn--primaire"
                >
                  {ui.ctaProjects}
                </button>,
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="night-btn night-btn--fantome"
                >
                  <AiOutlineGithub />
                  GitHub
                </a>,
                <Link
                  key="3d"
                  to="/3d"
                  className="night-btn night-btn--3d"
                  style={
                    reduit
                      ? undefined
                      : {
          ...
          <NightPortrait ui={ui} reduit={reduit} />
        </div>
      </FournisseurLampe>
    </section>
  );
