...
const ProDrawnTimeline = ({ etapes, statique = false }) => {
  ...
  return (
    <div ref={colonne} className={`pro-timeline day-timeline${dessine ? " is-drawn" : ""}`}>
      <div className="day-timeline__line" aria-hidden="true" />

      {etapes.map((step, i) => (
        <div
          key={step.title}
          className="pro-timeline__item"
          style={{ "--day-fx-delai": `${Math.round((retards?.[i] ?? 0) * DUREE_LIGNE)}ms` }}
        >
...
export default ProDrawnTimeline;
