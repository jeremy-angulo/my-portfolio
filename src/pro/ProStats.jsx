...
// Bandeau des chiffres clés. Les nombres comptent jusqu'à leur valeur, les
// filets qui séparent les colonnes se tracent du haut vers le bas, et les deux
// bords du bandeau s'ouvrent depuis le centre.
//
...
const Stat = ({ stat, index, vu }) => {
  ...
  return (
    <div className="pro-stat day-stat" style={{ "--day-fx-delai": `${index * 120}ms` }}>
      {index > 0 && (
        <span
          className="day-rule day-rule--v"
          aria-hidden="true"
          style={{ "--day-fx-delai": `${index * 100}ms` }}
        />
      )}
      <p className="pro-stat__value day-value">
        {fini ? (
          stat.value
        ) : (
          <>
            <span>{prefixe}</span>
            <span className="day-count" style={{ minWidth: `${chiffres.length}ch` }}>
              {compte}
            </span>
            <span>{suffixe}</span>
          </>
      ...
      <p className="pro-stat__label day-label">{stat.label}</p>
    </div>
  );
};

const ProStats = ({ statique = false }) => {
  ...
  return (
    <section
      ref={bandeau}
      className={`pro-stats day-stats${borde ? " is-lined" : ""}`}
      aria-label={proUi.statsLabel}
    >
      <span className="day-rule day-rule--h day-rule--haut" aria-hidden="true" />
      <div
        ref={rangee}
        className={`pro-container pro-stats__row day-stats__row${compte ? " is-in" : ""}`}
      >
        {proStats.map((stat, i) => (
